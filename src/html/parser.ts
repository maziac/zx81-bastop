 ...
 * @param valueHover (Optional) Is displayed on hovering over the 'value'.
 */
export function addRow(name: string, value: String | string | number = '', description = '', valueHover?: string | number) { // NOSONAR
 ...
 * section is expanded.
 */
export function addDetails(func: () => void, opened = false) {
	// Check if overridden
	if (overrideDetailsOpen != undefined) {
		opened = overrideDetailsOpen;
	}
 ...
 */
export function addMemDump(enableAscii = true) {
	let html = '';
	let asciiHtml = '';
	let prevNode;
	const LINECOUNT = 16;
...
document.addEventListener('contextmenu', function (event) {	// NOSONAR
	...
	// Check if target is the image/canvas.
	if (contextMenuTarget && contextMenuTarget.tagName === 'CANVAS') {
		// Show copy & save as for images
		contextMenuItemSaveAs.classList.remove("hidden");
	}
	else {
		// Show only copy for text
		contextMenuItemSaveAs.classList.add("hidden");
	}
...
contextMenuItemSaveAs.addEventListener('click', async function () {
	// Get data
	let data: string | Blob | ArrayBuffer | null = await getCopyData();
	if (!data)
		return;
 ...
 */
async function getCopyData(): Promise<string | Blob | null> {
	let data;
...
window.addEventListener('message', event => {	// NOSONAR
	...
	switch (message.command) {
		case 'setData':
			...
			// Store BASIC text string in global variable
			setTextData(message.zx81FilenameLength, message.zx81Filename, message.basicTxt);
			break;
		case 'setParser':
			// Store in global variable
